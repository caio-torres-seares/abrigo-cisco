      ...
      <div className="hidden md:flex w-64 bg-amber-100 flex-col">
        ...
        <nav className="flex-1 flex flex-col p-4 gap-1">
          <Link 
            ...
            className={`flex items-center gap-2 p-3 rounded-lg text-amber-800 ${location.pathname === '/funcionario/cadastrar-pet' ? 'bg-amber-200' : 'hover:bg-amber-200'}`}
          >
            <PlusCircle size={18} />
          ...
          <Link 
            ...
            className={`flex items-center gap-2 p-3 rounded-lg text-amber-800 ${location.pathname === '/funcionario/solicitacoes' ? 'bg-amber-200' : 'hover:bg-amber-200'}`}
          >
            <FileText size={18} />
          ...
          <Link 
            to="/funcionario" 
            className={`flex items-center gap-2 p-3 rounded-lg text-amber-800 ${location.pathname === '/funcionario' ? 'bg-amber-200' : 'hover:bg-amber-200'}`}
          >
            <Eye size={18} />
            <span>Ver Pets</span>
          </Link>
          <Link 
            ...
            className={`flex items-center gap-2 p-3 rounded-lg text-amber-800 ${location.pathname === '/funcionario/configuracoes' ? 'bg-amber-200' : 'hover:bg-amber-200'}`}
          >
            <Settings size={18} />
        ...
        <header className="bg-amber-100 p-4 flex justify-between items-center shadow-sm">
          ...
          <Sheet>
            <SheetTrigger asChild className="md:hidden">
              <Button variant="outline" size="icon" className="rounded-full border-amber-300">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-800">
          ...
          <div className="flex items-center gap-2 md:ml-4">
            <h1 className="text-amber-800 font-medium">{user?.name || 'Funcionário'}</h1>
          </div>
